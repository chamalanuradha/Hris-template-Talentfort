...
export default function Users() {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        const res = await axios.get('http://backend.chalanka.me/users');
  ...
  const handleSearch = async () => {
    try {
      const response = await axios.get('http://backend.chalanka.me/users');
      const { data } = response;
  ...
  const handleDelete = async (id) => {
    ...
    if (confirmDelete) {
      try {
        await axios.delete(`http://backend.chalanka.me/users/${id}`);
