...
export default function Updatecompany() {
  ...
  useEffect(() => {
    const fetchLoan = async () => {
      try {
        const response = await axios.get(`http://backend.chalanka.me/companies/${companyId}`);
  ...
  const handleClick = async (e) => {
    e.preventDefault();
    try {
      await axios.put(`http://backend.chalanka.me/companies/${companyId}`, companyData); // Pass companyData
      navigate('/companies');
    } catch (error) {
      console.log(error);
    }
